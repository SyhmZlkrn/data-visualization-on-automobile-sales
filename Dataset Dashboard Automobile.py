...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        # First, let's check the actual column name for unemployment rate
        unemp_col = None
        for col in recession_data.columns:
            if 'unemployment' in col.lower():
                unemp_col = col
                break
        
        if unemp_col:
            unemp_data = recession_data.groupby(['Vehicle_Type', unemp_col])['Automobile_Sales'].mean().reset_index()
            R_chart4 = dcc.Graph(
                figure=px.bar(unemp_data,
                    ...
                    color='Vehicle_Type',
                    barmode='group',
                    title=f'Effect of {unemp_col} on Vehicle Type and Sales'))
        else:
            # If no unemployment column found, use a placeholder
            R_chart4 = dcc.Graph(
                figure=px.bar(title='Unemployment data not available in dataset'))
